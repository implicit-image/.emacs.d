...
(defvar ii/gitignore-check-command "git --porcelain | grep .gitignore")
...
(defun ii/gitignore--should-recount (buffer)
  ...
  (with-current-buffer buffer
    (let ((default-directory (or (project-root (project-current nil))
                                 default-directory)))
      (not (string-empty-p (shell-command-to-string ii/gitignore-check-command))))))
...
(defun ii/gitignore-make-overlay (pos count max-len)
  (let* ((curr-len (- pos (pos-bol)))
    ...
    (overlay-put ov 'face `(:background ,(doom-color 'base2) :extend nil :foreground ,(doom-color 'yellow)))
    (overlay-put ov 'display (concat "" count "  "))
