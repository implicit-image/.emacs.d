...
;;; `use-package' customization
;; (defun use-package-normalize/:depends (name-symbol keywords args)
;;   "Accumulate dependency information by dependency type and package name."
;;   )
;;
;; (defun use-package-handler/:depends (name-symbol keyword archive-name rest state))
;;
;; (defun use-package-normalize/:treesit (_name keyword args)
;;   (use-package-as-one (symbol-name keyword) args 'use-package-normalize-paths))
;;
;; (defun use-package-handler/:treesit ())

(defmacro ii/add-hooks! (fn hooks)
  `(progn ,@(mapcar (lambda (hook)
                      `(add-hook ,hook ',fn))
                    hooks)))

(defmacro ii/define-prefix-command (&rest args)
  "Define prefix command."
  (let* ((cmd-var ))
    `(progn
       (define-prefix-command ,cmd-var ,map-var ,name)
       (bind-key ,binding ,map))))
